...
void USART3_IRQHandler(void)
{
    ...
    {
        if (USART_ReceiveData(USART3) == 0x80) {
            uflag = 1;
        }
        if (uflag) {
...
{
    u8 laser_on[5]      = {0x80, 0x06, 0x05, 0x01, 0x74};
    u8 laser_off[5]     = {0x80, 0x06, 0x05, 0x00, 0x75};
    u8 laser_measure[5] = {0x80, 0x06, 0x02, 0x78};
    ...
    if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_7) == 0) {
        ...
        beep_on_100ms();
        // flag = !flag;
        Usart_SendArray(USART3, laser_measure, 4);
        // if (flag) {
